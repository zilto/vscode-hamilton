...
function getCodeStyles(styles: string[]): CodeStyles {
  ...
  return htmlStyle
    ?.split(";")
    .map((str) => { const [key, val] = str.trim().split(":"); return {key, val} })
...
function handleUpdate(message: any){
  ...
  const moduleSet: Set<string> = new Set(message.details.elements.nodes.map((node: cytoscape.NodeDataDefinition) => node.data.module))
  ...
  transformNodes.forEach((target: cytoscape.NodeDataDefinition) => {
    ...
    if (rawNode){
      rawNode.incomers().nodes().forEach(incomer => 
        cy.add({data: {source: incomer.id(), target: target.data.id}})
      );
      cy.remove(rawNode);
    }
  })
