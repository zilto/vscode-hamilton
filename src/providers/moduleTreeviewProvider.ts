...
export class PythonFilesProvider implements vscode.TreeDataProvider<ModuleTreeItem> {
  public static readonly viewId = "hamilton.pythonFiles_treeview";

  private _onDidChangeTreeData: vscode.EventEmitter<ModuleTreeItem | undefined | null | void> = new vscode.EventEmitter<
...
export class ModulesProvider implements vscode.TreeDataProvider<ModuleTreeItem> {
  ...
  async getChildren(element?: ModuleTreeItem): Promise<ModuleTreeItem[]> {
    ...
    // triggers on recursive calls; get all python functions in python files
      ...
      return filteredSymbols.map((s) => ({
        label: s.name,
        picked: true,
        uri: vscode.Uri.from(element.uri),
        type: "moduleFunc",
        symbol: s,
        collapsibleState: vscode.TreeItemCollapsibleState.None,
      }));
